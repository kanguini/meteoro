...
function Gallery({ initial, library }: { initial: GalleryItem[]; library: { url: string; filename: string }[] }) {
  const [rows, setRows] = useState<GalleryItem[]>(initial);
    ...
    <div style={{ display: 'grid', gap: '0.9rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1, display: 'grid', gap: '0.5rem', minWidth: 0 }}>
              <input
                ...
                value={row.url}
                onChange={(event) =>
                  setRows(rows.map((item, position) => (position === index ? { ...item, url: event.target.value } : item)))
                }
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
        <button
          ...
          onClick={() => setRows([...rows, { url: '', altPt: '', altEn: '' }])}
        >
          Acrescentar imagem
        </button>
        ...
        {library.length > 0 && (
          <select
            ...
            onChange={(event) => {
              ...
              setRows([...rows, { url: event.target.value, altPt: '', altEn: '' }]);
            }}
